...
function renderStateContract(stateFilePath?: string): string[] {
  ...
  return [
    ...
    "- 默认主回复面是一张 CardKit 流式卡片:bridge 起手只显示一行“努力回答中...”,随后把 assistant 文本增量作为运行中答案流写入同一张卡,完成后收敛成干净总结卡。",
    "- 运行中不会展示 raw tool_use/tool_result;本地路径、token/secret/env 形态会被 bridge 做保守脱敏。",
    "- 你负责把最终给运营看的正文、状态、下一步问题、是否需要 choices/结构化卡片写进 state.json。",
    `你不直接发/编辑 bridge 管理的 post 或卡片;你只写 ${stateTarget},bridge 读它来做安全渲染。`,
    "",
    "答案流通道:",
    "- 默认不需要手动写答案 marker;普通 assistant 文本会自动作为运行中答案流。",
    `- 如需精确限定“只流这一段”,可把该段正文包在独立行 marker 之间: \`${ANSWER_BEGIN_MARKER}\` 到 \`${ANSWER_END_MARKER}\`。`,
    "- marker 是可选 override,不是每个 bot 必须手写的格式。",
