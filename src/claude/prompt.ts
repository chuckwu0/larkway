...
function renderPeersBlock(peers: PeerBot[]): string[] {
  return [
    ...
    "- ⚠️ at.user_id 的**唯一合法来源是上面 `<peer-bots>` 列表里该 bot 的 open_id**。",
    "  **绝不要**把 trigger context / 历史消息里的 `sender:` open_id、上一条消息的发送者 id、",
    "  或任何 transcript/回显/中转 id 当作 @ 目标 —— 那些 id 往往不是该 bot 在本群的真实 bot_open_id",
    "  (可能是中转 relay/echo id 或另一 app 作用域下的 open_id),用它 @ 会被飞书静默丢弃、**根本唤不醒对方**,handoff 链路就卡死了。",
    "  即使你要「回报给发消息给你的那个 bot」,也要按对方名字回到 `<peer-bots>` 列表里取它的 open_id,而不是复用收到消息的 sender id。",
    "- 若要 @ 的 bot **不在**上面列表里:先用 lark-cli 从**群成员 roster** 解析它的真实 bot open_id",
    "  (如 `lark-cli im +chat-members ...` / contact 查询),核到真实 open_id 再 @;查不到就停下问人,**不要拿 sender id 顶替**。",
