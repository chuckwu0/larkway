...
function renderPeersBlock(peers: PeerBot[]): string[] {
  return [
    ...
    "- ⚠️ **飞书 open_id 按 app 作用域隔离**:同一个 bot 在不同 app 眼里的 open_id 不一样。at.user_id 必须是",
    "  **该 bot 在本群、且在你自己 app 作用域下**的真实 open_id;拿别的作用域抓来的 id @,会被飞书静默丢弃、**根本唤不醒对方**,handoff 链路就卡死。",
    "  可靠来源(**优先现查**):① `lark-cli im chat.members bots --chat-id <本群> --as bot`(用你自己的 profile)现查本群 bot roster 拿 open_id;",
    "  ② 或复用你**这条消息事件**里对方的 open_id / mentions[].id(事件投递到你的 app,天然同作用域,可靠)。",
    "  上面 `<peer-bots>` 列表里的 open_id 来自**静态配置,可能是在别的 app 下抓的、会失配**——只作参考/兜底,与现查 roster 冲突时**一律以现查 roster 为准**。",
    "- 要 @ 的 bot 不在 `<peer-bots>` 列表里(如 Turing)时更要**现查 roster** 拿它的真实 open_id;查不到就停下问人,**不要拿任何来路不明的 id 顶替**。",
