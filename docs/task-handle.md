...
> 状态:**v5**(2026-07,五信号委托契约,见 §16;建卡从人肉动作变成 agent 自判 + bridge 机械执行)。本文是该 feature 的权威设计文档;实现以此为准。
> 原则依据:[principles.md](principles.md)。飞书平台能力结论均经过实测验证(见 §9 平台事实)。
>
> ## ⚠️ 当前口径速查(先读这段再往下翻)
>
> 本文按版本增量累积书写,**越靠前的章节越旧**。下面三条是当前生效口径,与前面章节冲突时以本段为准:
>
> 1. **零配置**。主路径不需要清单、不需要 `tasklist-init`、不需要把 bot 加成 editor——任务分享到 bot 所在群自带的读+评论权限就够(§15.5)。装配是无条件的(`main.ts`:per-bot 的 client/store/writeback/claim/CommentPoller/StallDetector 常在,空 claim store = 零网络调用)。
> 2. **agent 可以自己建任务**(v5 §16):agent 判断「值得追踪」→ 在 `state.json` 写 `task_handle.create` → bridge 机械建卡 + 话题回链写进描述 + **把发起人加为关注人** + 自动绑定本话题。此后 `note`/`due`/`blocked`/`done` 都落任务评论(评论有推送,描述变更没有)。**这条覆盖了 §15.6 的「❌ 不自动建任务」**。
> 3. **清单 / `tasklist-init` / 候选发现 / auto-bind / 描述回写 = 辅路径(§4 话题转任务)遗产**(§15.5)。继续服务已部署用户,**不再是 onboarding 路径**。读 §5–§7、§9.9–§9.11、§14.1 时请带着这个前提。
>
> 三条建立句柄的路径,按当前推荐度排序:**① agent 自建(v5 §16,零配置,推荐)** → ② 人派单(v4 §15:建任务 → 发送到会话 → 群里 @ agent) → ③ 话题转任务(§4 辅路径遗产,需清单)。
>
...
**群里的一个飞书话题 = agent 的一项工作;飞书任务 = 这项工作的管理句柄** —— 可改名、可备注、看得到状态、收得到推送、点得回话题。

建立句柄有三条路径,认领之后的生命周期维护完全相同:

| | 路径 | 谁发起 | 需要清单? |
|---|---|---|---|
| ① **v5 主路径** | **agent 自建**(§16):agent 判断值得追踪 → 声明 `task_handle.create` → bridge 建卡 + 回链 + 把发起人加为关注人 + 自动绑定 | agent | ❌ |
| ② v4 主路径 | 任务派单(§15):人建任务 →「发送任务到会话」分享进群 → 话题里 @ 某个 agent,谁被 @ 谁认领 | 人 | ❌ |
| ③ 辅路径(遗产) | 话题转任务(§4):已经聊出来的活,右键转任务挂上句柄,agent 每轮静默自动认领 | 人 | ✅ |

「Agent Team」共享清单只服务路径 ③(候选发现 / auto-bind / 全景看板),**v4.1 起已退出主叙事**(§15.5)。路径 ①② 零配置。
...
3. **真正的开关 = 有没有清单,不是配置字段**(v2;**v4 修订:开关变成「有没有认领」**):去掉 `taskHandle.enabled`。bot 在 startup 时只做只读解析(yaml 里的 `tasklistGuid`,或共享注册文件里已有的 guid)——**清单本身只由 §7 的 `tasklist-init` CLI 建一次,bot 自己从不自动建**(F1 修正:早期设计曾让 bot 在 startup 时自动 createTasklist,已删除——没有 owner 身份的自动建清单既建不出 owner 能看到的板,又让每个 bot 每次启动都发一次可能失败的网络调用;二者都无必要)。省略 `taskHandle:`/两处都查不到 guid → 清单侧功能(候选/自动绑定/self-join)休眠,**零网络调用**。⚠️ 实现铁律:降级必须**密不透风**——bot 唯一还会发的 task API 调用是「已知 guid 时把自己加为 editor」(幂等 self-join),这个调用的任何失败也绝不能冒出错误卡/刷屏。
   **v4 语义变化(有意为之,运维需知)**:主路径不依赖清单,task 机件(store/writeback/CommentPoller/StallDetector)改为无条件构造——休眠契约从「无 guid = 全体休眠」收窄为「**无认领 = 零网络调用**」。这意味着 pre-v4 的「删掉 tasklistGuid 即整体停用」杠杆不再成立:磁盘上仍有活跃认领(`<LARKWAY_HOME>/<botId>/task-handles.json`)时,删 guid 只停清单侧功能,已认领任务的评论轮询和停滞巡检**照常继续**(它们按 guid 直连任务,不经清单)。想彻底停用:完成/删除对应任务(映射自清)或删掉该 bot 的 task-handles.json;只想关巡检:`stallDetectionDisabled: true`。
...
## 7. 配置与权限(v2;**清单部分已是辅路径遗产**)

> ⚠️ **别把本节当 onboarding**(v4.1/v5 修订,见顶部速查):`tasklist-init`、共享清单、把 bot 加成 editor
> **只服务辅路径 ③(话题转任务)的候选发现 / auto-bind,以及 owner 想要的全景看板**。主路径 ①(agent 自建)
> 和 ②(人派单)**零配置**,跳过本节的 provisioning 全段即可。
>
> 主路径实际需要的只有两样:**① 飞书 scope**——路径 ② 只要「任务分享到群」自带的读+评论;路径 ①
> 额外需要 `task:task:write`(建任务 + 加关注人),缺了是 best-effort 静默降级成一行诊断,不会报错。
> **② 判据对齐**——prompt 契约的默认判据是「值得追踪才建卡,一问一答不建卡」;某个 bot 想让每次正式交付
> 都建卡,把这条规则写进它的 L2 memory,不要改 bridge。

```yaml
# bots/<bot>.yaml 新增(可选;不配 = 辅路径的清单侧功能休眠,主路径不受影响)
taskHandle:
...
## 16. v5:声明式任务信号(五信号委托契约)

> 2026-07-18。产品动机:人把活委托给 agent 时关心的信号,和委托给人完全一致——
> **接了 / 进展 / 什么时候完 / 完成或失败 / 堵住需要人**。v4 只有 2、4 有管道
> (note/done),v5 补齐 1、3、5,并把「建卡」这个入口从人肉动作变成
> **agent 自判 + bridge 机械执行**(与 handoffs 同款声明式模式)。
> 除这五个信号外,什么都不上任务侧——日志/工具过程/内部往返,人不关心。

### 16.1 state.json 声明面(全部按需、互不依赖)

```jsonc
"task_handle": {
  // 信号1 接了:agent 判断「值得追踪」(三个月后复盘会找它 / 执行跨越本轮对话)
  // 时声明;bridge 建卡 + 话题回链写进描述 + 发起人加为 follower + 自动认领。
  // 一问一答不建卡。
  "create": { "summary": "…", "due": "2026-07-22T18:00:00+08:00" },
  // 或认领已存在任务(v4 语义不变;v5 起 guid 可选)
  "guid": "…",
  // 信号2 进展(v4 已有):一句话里程碑,只在实质进展时写
  "note": "…",
  // 信号3 ETA:设期/改期;改期必须带原因,bridge 落评论说明
  "due": "2026-07-23", "due_reason": "等数据源拍板耗时半天",
  // 信号5 阻塞:bridge 落 🚧 评论,任务关注人收到推送(这就是送达机制,
  // 无需 @;评论推送 > 描述变更这一平台事实见 §9)
  "blocked": "付费 API 配额用尽,需要选降级方案",
  // 信号4 终态(v4 已有)
  "done": true
}
```
...
建卡描述第一行必须是话题回链。平台事实:深链只能从 `omt_*` 真话题 ID 构造,
`om_*` 消息 ID 构造的链接**点击即死**(§15.3 已知)。create 路径先取事件自带
thread_id,取不到再做一次 messageLookup refresh;仍取不到 → **显式降级**为
群链接 + 「话题深链不可用」说明,绝不静默缺失。

### 16.3 边界

- 判断永远在 agent 侧(该不该建卡、改期是否合理),bridge 只机械执行——薄通道不变。
- 全部 best-effort:任一信号失败降级为 runtime event 诊断行,不影响本轮交付。
- p2p sticky session(合成 threadId)建的卡拿不到话题回链,走群链接降级(§F1 已知取舍)。
- 交付双指针是 prompt 契约里的 agent 侧规范(结论报告导成飞书 doc,doc 链接在前、
  本地路径在后),不是 bridge 机制。

## 17. v5.1:建卡率修复 + 评论回流修复(BL-49,2026-07-27)

> 起因:2026-07-27 在本机 codex bot 上做的一次真机 dogfood。结论——**v5 的两条关键
> 链路都不达标**,而且两个问题都是**静默**的,不看日志/落盘根本发现不了。

### 17.1 实测发现

| 现象 | 根因 | 严重度 |
|---|---|---|
| 明确说了「后面我会分几次回来追进度」,agent 仍判「一问一答」不建卡;直到用户直白说「开成一条飞书任务」才建 | 判据要求 agent **预测未来**,而决策点在 turn 1 结束——证据最弱的时刻 | 概率性 |
| 用户在任务评论区留言,agent **永远收不到** | `selectNewComments` 在 `lastSeenCommentId === undefined` 时只设基线、零 emit。新建任务的评论历史必然为空 → **第一条评论必被当历史吞掉** | **确定性 bug** |

第二条对 v5 尤其致命:辅路径 adopt 老任务时「不重放历史」是对的,但 v5 的任务是
bridge 刚建的,第一条评论就是用户的真实反馈。

### 17.2 修法(已实现)

1. **评论基线改用 `claimedTs`**(`commentPoller.ts`):无游标时只 emit
   `createMillis > claimedTs` 的评论。adopt 老任务照样跳过历史,新建任务的第一条
   评论不再丢。时间戳缺失的评论算历史(保守,与修前同方向)。
2. **判据从预测改成事实**(`prompt.ts` + `handler.ts`,**两轮才调准**):bridge 注入两条机械事实
   `thread_turn_count`(含本轮)和 `thread_has_task_card`——数据本来就在
   `SessionRecord.turnCount` 里,零新增开销。判据改为「**已跨轮(≥2)且尚未建卡 →
   默认建卡**」。用户回来追第二轮,是**已发生的事实**,不需要预知未来。
3. **代价对称化**(`prompt.ts`):原文只写了滥建的代价,补上漏建的代价(「该建没建 =
   用户永久失去追踪入口和推送,且没有任何人会发现」)。修前的损失函数是单边的,
   agent 保守是必然结果。
4. **主路径黑洞诊断**(`handler.ts`,`TASK_CARD_BLACKHOLE_TURNS = 4`):≥4 轮仍无
   任务卡时记一条 runtime event。辅路径早有候选黑洞提示(§14.1),主路径此前是纯
   黑箱——**这才是建卡率低长期没被发现的原因**。仅诊断,不打扰用户、不替 agent 决策。

### 17.4 v5 自建卡的维护语义(同一轮 dogfood 追加发现)

真机复验时发现的第三个问题,一个根因带三个症状:**bridge 自建的卡没有被标成
comment 模式**(`mode` 此前只在「认领的任务 == 本话题根消息分享的那条」时才设),
于是它落回 v4.1 之前的全量 writeback:

1. **自动勾完成** —— agent 写 `done: true` 就直接把任务勾了。用户第一次看到这条
   任务时它**已经是 done**,「看过请点完成」这个人工确认动作整个消失。
2. **往描述里写状态块/进展日志** —— v4.1 已明令不写(描述变更**不推送**,评论才推送;
   写了也没人看)。
3. **自动 reopen**。

修法:`claimedTaskGuid === bridgeCreatedTaskGuid` 时同样标 `mode: "comment"`。
v4.1 的理由(turn 成功 ≠ 交付、完成属于人)与卡是谁开的无关,语义应当路径无关。

> 附带修掉一个真机渲染问题:描述状态块用的 Markdown 加粗(`**状态**:进行中`)在飞书
> 任务描述里**不渲染、且标签被吃掉**,真机显示成孤零零的 `:进行中 (in_progress)`。
> 改成 comment 模式后描述压根不再写,问题随之消失。

### 17.5 判据措辞的第二轮修正(真机 n=3 才收敛)

第一版判据(「跨轮 → 默认建卡」)上线后真机复验 **1 建 1 不建**。事实确实注入了
(`turnCount=2` 落盘可证),但同一段里「一问一答聊完就完的**不建卡**」还在,而那两次
第 2 轮**恰好都是一轮就答完的** —— 两条规则互相抵消,agent 用了后一条。

修正:把一问一答豁免**显式限定在 `thread_turn_count: 1`**,并写明「『我这一轮一句话
就答完了』不是不建的理由 —— 判据看的是话题跨了几轮,不是本轮答没答完」。

教训值得记:**给 agent 的规则不能只加不减**。新规则和旧规则同处一段时,旧规则不会
自动让位;必须显式限定旧规则的适用范围,否则等于什么都没改。

### 17.6 平台事实:任务描述的 Markdown 支持面(2026-07-27 真机探针实测)

往一条真实任务的 `description` 写入样本、在飞书客户端肉眼比对,结论:

| 语法 | 渲染结果 | 能不能用 |
|---|---|---|
| `[文字](https://example.com/reference)` | **渲染成可点链接** | ✅ 用 |
| `**加粗**` / `*斜体*` | **标记连同被包住的文字一起消失** | ⛔ 禁用(会丢内容) |
| `` `行内代码` `` | 反引号被吃,文字保留 | ⚠️ 无收益,别用 |
| `- 列表项` | 原样显示 `- 列表项`,不成列表 | ⚠️ 安全,当纯文本用 |
| 裸 URL | 自动变蓝,但 applink 长链会折成三行 | 可读性差 |

由此修掉两处:

1. **建卡描述的话题回链改用 `[点击进入工作话题](https://example.com/topic)`** —— 原先一条裸 applink 长链在
   客户端折成三行,是本次 dogfood 里用户第一眼吐槽的"一坨"。
2. **状态块去掉所有 `**` 加粗** —— v3 那次「人类友好化」把标签写成 `**状态**:进行中`,
   而加粗会被吞掉,**线上所有走辅路径的用户从那时起看到的都是丢了标签的 `:进行中
   (in_progress)`**。`parseStatusSnapshotStatus` 同时兼容新旧两种写法,否则老任务的状态
   会突然读不出来。

> 教训与 §17.5 同源:**"更好看的排版"没在真机上看过就上线,等于没做**。两次都是同一个
> 改动(v3 人类友好化)埋的雷,一次是规则打架,一次是渲染假设。

### 17.7 COT 气泡漏到群顶层:合成 turn 的锚点

用户在群主流里看到本该待在话题里的「任务已完成」气泡。排查结论分两半:

**① `10002` 不是权限问题(推翻既有假设)。** `cotProgress.ts` 原注释把话题通道的
`10002 Bot/User can NOT be out of the chat` 记成"本租户拒绝"。控制变量实测:传一个
**根本不存在的 `omt_` id,报的是同一个 10002**(传 `om_` 则是 10001 格式错)。说明 API
压根没走到成员检查那步——`receive_id_type=thread_id` 只是**解析不了普通群里的回复串**
(本群 `chat_mode=group`)。它多半只在真话题群(`chat_mode=thread`)成立,但该租户下无
话题群可验,**这半句留待验证**。所以降级到 chat 通道是预期行为,不是缺陷。

**② 漏出去的只有合成 turn。** 真人触发的轮次气泡都在话题内(抽查 `thread_id` 为
`omt_`);主流里那几条 `thread_id` 为空的,对照 runtime event 全部来自 **`[停滞提醒]`
巡检唤醒**。机制:合成 turn 的 `message_id` 是伪造串、`thread_id` 是 session key(`om_`
根消息 id),于是 `parsed.messageId` 落到 `reply_anchor_message_id` = **话题根消息**,而
锚在根消息(首楼 pos=-1)正是"在群顶层 quote-reply"那个已实测的分支。

修法:合成 turn 与新话题同等对待——跳过前置建气泡,落到卡片之后的站点,锚在话题内的
那张卡上。

> 同一个坑修过一次:`main.ts` 的 `reply_anchor_message_id` 就是为合成 turn 的**回复**
> 加的。根消息对回复是对的锚点,对气泡是错的——**一个字段被两条链路以相反的方式需要**,
> 而修第一条时没人想到第二条。

### 17.3 边界不变

判断仍然 100% 在 agent 侧:bridge 注入的是可观测事实(轮次、有没有卡),不是「你
应该建卡」的指令,也没有任何 bridge 侧的自动建卡。§16.3 的边界一条没动。
