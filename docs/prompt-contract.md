...
|---|---|---|
| 外壳 | 创建卡片、节流 PATCH、失败兜底、崩溃后 reconcile | 不直接 PATCH 卡片 |
| 头部 | 通用状态:处理中 / 已回复 / 完成 / 出错 | 可选写 `card_title` / `card_color` 覆盖本轮语义 |
| 正文 | 安全渲染 markdown、分片、清理工具调用泄漏 | 写 `last_message`,决定最终给运营看的内容 |
| 工具区 | 运行中展示最近工具调用摘要,用于证明还在工作 | 不依赖工具区表达业务阶段 |
