...
| v0.3.37 | v0.3.37 | 当前 patch / 已发布 | Actually silence vendored node-sdk raw AxiosError dumps: loggerLevel:fatal is dead code (fatal===0 is falsy, coerced back to info by the SDK) — inject a no-op logger on the four bare task REST clients instead, and print compact error text in our own warns. Regression-locked against reverting to loggerLevel. |
